...
- `aggregation_excluded_user_ids` に含まれるユーザーはランキング集計対象外
- 以前の `target_user_ids` フィルタは撤廃済み
- `maintenance_enabled=true` の間は定時処理を停止（`maintenance_until_epoch` 到達で自動解除）
...
- `/study weekly_rank [limit]` 週次ランキングの途中結果を表示（1〜30、デフォルト10）
- `/study timer <minutes> [title]` 実行したテキストチャンネルにタイマー終了通知を送信
- `/study timer_cancel` 自分が起動したタイマーを停止

### 管理者または許可ユーザー

管理者権限がない場合は、`config.json` の `command_allowed_user_ids` にユーザーIDを追加してください。
...
- `/config maintenance set <true/false>` 定時処理停止スイッチ
- `/config maintenance set_for <minutes>` 実行時刻から指定分だけメンテナンスモード有効化
- `/config aggregate add_exclude_user <member>` 集計除外ユーザー追加
- `/config aggregate remove_exclude_user <member>` 集計除外ユーザー削除
- `/config aggregate clear_exclude_users` 集計除外ユーザー全削除
...
- `aggregation_excluded_user_ids` 既定: `[]`
- `command_allowed_user_ids` 既定: `[]`
- `maintenance_enabled` 既定: `false`
- `maintenance_until_epoch` 既定: `0`
